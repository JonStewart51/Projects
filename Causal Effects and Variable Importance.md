...
The above table shows that, **V7** has by far the largest positive causal effect on the probability of type II diabetes, while **V5** seems to have a strong causal negative effect on the likelihood of diabetes. Again, note the differences between the causal estimations and the variable importance estimations. The reason is somewhat obvious, and also underlines a common misapplication of tree based variable importance measures. To a tree, the most important variables are the ones that, when forming a node, causes the greatest decrease in mean squared error or some entropy based splitting measure. This is a very different question than to what degree a variable drives a process, where significance is determined by the differences between the data points within each leaf, using a distance measure or statistical test to determine the amount of difference between treatment and control points.

The above report is a toy problem essentially, as this is a very well studied data set, but this should give a better idea regarding the ability of causal forests to identify the important drivers of processes, and also show the pitfalls of using how a tree determines variable importance as a proxy of how important that variable is for driving a given process. Additionally, the general concept can be extended to multiple time series, where Granger causality variants have trouble, or other scenarios, with only minor modifications.
